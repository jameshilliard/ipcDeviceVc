...
void d3deskey(unsigned char *key,short edf)	/* Thanks to James Gillogly & Phil Karn! */
{
    register int i, j, l, m, n;
    unsigned char pc1m[56] = {0};
    unsigned char pcr[56] = {0};
    unsigned long kn[32] = {0};
...
void des2key(unsigned char *hexkey, short mode)		/* stomps on Kn3 too */
{
	...
	d3deskey(&hexkey[8], revmod);
	cpkey(KnR);
	d3deskey(hexkey, mode);
	cpkey(Kn3);					/* Kn3 = KnL */
	return;
}
